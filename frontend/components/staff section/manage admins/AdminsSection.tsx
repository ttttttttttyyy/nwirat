      ...
      <div className="grid gap-6 xl:grid-cols-[430px_1fr]">
        <div className="rounded-[2rem] border border-slate-200 bg-white shadow-sm">
          ...
          <div className="p-5">
            ...
            <button onClick={grantAdminAccess} disabled={adminEmailState !== 'valid'} className="mt-6 h-12 w-full rounded-2xl bg-[#064e3b] font-black text-white shadow-lg shadow-emerald-900/20 transition hover:bg-[#065f46] disabled:opacity-50">Enregistrer les Droits</button>
          </div>
        </div>
              ...
              <div key={user.id} className="rounded-3xl border border-slate-100 bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0"><p className="truncate font-black text-slate-900">{user.name || 'Personnel sans nom'}</p><p className="truncate text-sm font-semibold text-slate-500">{user.email}</p></div>
                  </div>
                  <span className="shrink-0 rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600">
                    {user.role === 'ROLE_ADMIN' ? 'Admin' : user.role === 'ROLE_AGENT' ? 'Agent' : user.role === 'ROLE_DRIVER' ? 'Chauffeur' : user.role}
                  </span>
                </div>
                <p className="mt-3 rounded-2xl bg-emerald-50 px-3 py-2 text-xs font-black text-[#064e3b]">{user.servicePermissions || 'Accès complet'}</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  <button onClick={() => loadAdminForEdit(user)} className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-600">Modifier les droits</button>
                  {user.role !== 'ROLE_DRIVER' && <button onClick={() => grantDriverRole(user.email)} className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-black text-[#064e3b]">Rendre chauffeur</button>}
                  {user.role === 'ROLE_AGENT' && <button onClick={() => removeAdminRole(user.email)} className="rounded-full bg-rose-50 px-3 py-1.5 text-xs font-black text-rose-700">Retirer admin</button>}
                  {user.role === 'ROLE_DRIVER' && <button onClick={() => removeDriverRole(user.email)} className="rounded-full bg-rose-50 px-3 py-1.5 text-xs font-black text-rose-700">Retirer chauffeur</button>}
                </div>
              </div>
            ))}
